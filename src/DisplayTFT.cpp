...
DisplayTFT::DisplayTFT()
{
    ...
    // --- ESP32 PWM / Backlight configuratie (Arduino Core v2.x) ---
    #if defined(ESP32)
      ledcSetup(0, 5000, 8); // Kanaal 0, 5kHz, 8-bit resolutie
      ledcAttachPin(BRIGHTNESS_PIN, 0);
    #endif

    brightness = 100; // Startwaarde (procent)
...
{
    brightness = percent;
    // Bereken de waarde op basis van 0 t/m 255
    int dutyCycle = (MAX_BRIGHTNESS_VALUE * percent) / 100;
    
    #if defined(ESP32)
      ledcWrite(0, dutyCycle); // Schrijf naar PWM kanaal 0
    #else
      analogWrite(BRIGHTNESS_PIN, dutyCycle);
    #endif
}
...
void DisplayTFT::drawOctoPrintStatus(OctoPrintMonitorData* printData, String printerName, bool enabled)
{
    ...
    if(!enabled)
    {
        ...
        drawPrinterNotEnabled(printerName);
